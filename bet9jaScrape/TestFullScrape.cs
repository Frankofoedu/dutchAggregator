...
namespace bet9jaScrape
{
    public class TestFullScrape
    {
            ...
            using (var driver = new ChromeDriver())
            {
                ...
                while (curindex < total)
                {
                    ...
                    if (webElements[curindex].GetAttribute("class").Contains("firstItemGroup"))
                    {
                        ...
                        sb = new Bet9ja() { League = GetLeague(webElements[curindex]), Matches = new List<Matches>() };
        ...
        Matches GetMatch(IWebElement element)
        {
            ...
            foreach (var oddGroup in oddDivs)
            {
                ...
                foreach (var odd in odds)
                {
                    ...
                    var SnO = new Odds() { Type=type, Selection = sNoDivs[0].Text.Trim(), Value = sNoDivs[1].Text.Trim() };
